"""Benchmark TurboQuant + PQ residual hybrid on BigEarthNet (269K vectors)."""
...
for model_name in ['prithvi', 'remoteclip']:
    emb_path = EMBED_DIR / f'{model_name}_bigearthnet.npz'
    ...
    print(f'\n{"="*60}')
    print(f'{model_name}/bigearthnet: {embeddings.shape}')
    print(f'{"="*60}')
    ...
    for cfg in CONFIGS:
        for seed in SEEDS:
            ...
            print(f'  {cfg["name"]} seed={seed} '
                  f'(train={len(train_emb)}, db={len(database)}, q={n_queries})...',
                  end=' ', flush=True)
            ...
            # Train PQ on residuals from training set (216K vectors)
            ...
            r = {
                'model': model_name, 'dataset': 'bigearthnet',
            ...
            print(f'R@1={recall["1"]:.3f} R@10={recall["10"]:.3f} '
                  f'R@100={recall["100"]:.3f} '
                  f'({bpv:.0f} B/vec, train={train_time:.1f}s)')

# Save
out = RESULTS_DIR / 'residual_hybrid_ben_results.json'
